...
$query_peserta = mysqli_query($koneksi, "
    SELECT p.*, w.nama, d.qr_token, d.is_used 
    FROM peserta p
    LEFT JOIN warga w ON p.nik = w.nik
    LEFT JOIN data d ON p.id_peserta = d.id_peserta
    ORDER BY p.id_peserta ASC
");

// Query untuk mengisi dropdown NIK di form modal
$data_warga = mysqli_query($koneksi, "SELECT nik, nama FROM warga ORDER BY nama ASC");
?>
    ...
    <table class="table table-bordered table-hover table-striped mt-4">
        <thead class="table-dark">
            <tr>
                ...
                <th class="text-center">Daging (kg)</th>
                <th class="text-center">Status Ambil</th>
                <th>QR Token</th>
                <th class="text-center">QR Used</th>
        ...
        <tbody id="tabelPesertaBody">
            ...
            ?>
             <tr id="row-peserta-<?= $row['id_peserta'] ?>">
                ...
                <td class="text-center"><?= htmlspecialchars($row['jumlah_daging_kg']) ?> kg</td>
                <td class="text-center">
                    <span class="badge <?= $row['status_ambil'] === 'sudah' ? 'bg-success' : 'bg-warning text-dark' ?>">
                        <?= ucfirst($row['status_ambil']) ?>
                    </span>
                </td>
                <td><?= htmlspecialchars($row['qr_token'] ?? '-') ?></td>
                <td class="text-center">
                    <?php if (isset($row['is_used']) && $row['is_used'] == 1): ?>
                        <span class="badge bg-success"><i class="bi bi-check-circle-fill"></i> Sudah</span>
                    <?php else: ?>
                        <span class="badge bg-secondary"><i class="bi bi-x-circle-fill"></i> Belum</span>
                    <?php endif; ?>
                </td>
                <td class="text-center">
      ...
      <div class="modal-body">
        <form id="formPeserta">
            ...
            <div class="mb-3">
                ...
                <input type="number" id="jumlah_daging_kg" name="jumlah_daging_kg" step="0.1" class="form-control" required>
            </div>
            <div class="mb-3">
...
document.addEventListener('DOMContentLoaded', function() {
    // ... (Definisi variabel modal, form, dll. tetap sama) ...
    ...
    document.getElementById('btnTambahPeserta').addEventListener('click', function() {
        // ... (Kode untuk tombol tambah tetap sama) ...
        ...
        nikSelect.required = true; // Set required saat form tambah
    ...
    tabelPesertaBody.addEventListener('click', function(event) {
        // ... (Kode event listener click tetap sama, kita hanya ubah bagian 'btn-edit-peserta') ...
            ...
            fetch(`fungsi/get_peserta_detail.php?id=${id}`)
                ...
                .then(data => {
                    if (data.status === 'success') {
                        // ... (kode untuk mengisi form edit tetap sama) ...
                        ...
                        nikSelect.required = false; // HAPUS ATRIBUT REQUIRED SAAT DI-HIDE
        ...
        // ... (Kode untuk 'btn-hapus-peserta' dan 'formPeserta.submit' tetap sama) ...
