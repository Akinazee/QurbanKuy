...
$query_keuangan = mysqli_query($koneksi, "
  SELECT k.*, p.nik, w.nama AS nama_warga, perl.nama_perlengkapan 
  FROM keuangan k
...
$query_peserta = mysqli_query($koneksi, "SELECT peserta.id_peserta, peserta.level, warga.nama FROM peserta JOIN warga ON peserta.nik = warga.nik WHERE peserta.level = 'berqurban' ORDER BY warga.nama ASC");
                ...
                <select name="id_peserta" id="id_peserta" class="form-select">
                    ...
                    <?php while ($row = mysqli_fetch_assoc($query_peserta)) : ?>
                        <option value="<?= htmlspecialchars($row['id_peserta']); ?>">
                            <?= "ID: " . htmlspecialchars($row['id_peserta']) . " - " . htmlspecialchars($row['nama']) ?>
                        </option>
                    <?php endwhile; ?>
                </select>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Mengambil elemen-elemen form dari dalam modal
    const jenisSelect = document.getElementById('jenis');
    const idPerlengkapanSelect = document.getElementById('id_perlengkapan');
    const jumlahInput = document.getElementById('jumlah');
    const keteranganTextarea = document.getElementById('keterangan');
    
    // Data perlengkapan dari PHP
    const perlengkapanData = <?= json_encode($perlengkapan_array); ?>;

    // Fungsi ini sekarang sama persis dengan yang ada di form_keuangan.php original
